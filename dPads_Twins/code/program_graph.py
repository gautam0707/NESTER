...
class ProgramGraph(nn.Module):
    ...
    def execute_graph(self, batch, output_type, output_size, device, clear_temp=True, cur_arch_train=False):
        ...
        batch_input = torch.tensor(batch,dtype=torch.float32) #[torch.tensor(traj) for traj in batch]
        batch_padded, batch_lens = pad_minibatch(batch_input, num_features=batch_input[0].size(0))
        batch_padded = batch_padded.to(device)

        root_program = list(self.root_node.prog_dict.values())[0][0]
        type_sign = list(self.root_node.prog_dict.keys())[0]
        out_padded = self.root_node.execute_on_batch(type_sign=type_sign, prog_ids=[0], batch=batch_input, batch_lens=None)
