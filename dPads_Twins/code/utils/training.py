...
from utils.data_loader import pad_minibatch, unpad_minibatch, flatten_tensor, flatten_batch
...
#     batch_padded, batch_lens = pad_minibatch(batch_input, num_features=batch_input[0].size(0))
#     batch_padded = batch_padded.to(device)
    out_padded = program.execute_on_batch(torch.tensor(batch, dtype=torch.float), None)
    if output_type == "list":
    ...
    for epoch in range(1, num_epochs+1):
        for batchidx in range(len(trainset)):
            ...
            if config.globalmmdloss:
                lamba = 0.5
                loss = lossfxn(predicted_vals, true_vals)+lamba*config.globalmmdloss
                config.globalmmdloss = 0
            if config.globalpropensityloss:
                lamba = 0.5
                loss += lamba*config.globalpropensityloss
                config.globalpropensityloss = 0
