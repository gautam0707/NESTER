...
def compute_pehe_score(best_program, test_data, test_labels, output_type, output_size, num_labels, device='cpu',verbose=False):
    ...
    predicted_vals1 = process_batch(best_program, test_input, output_type, output_size, device=device)
    ...
    predicted_vals2 = process_batch(best_program, test_input, output_type, output_size, device=device)

    t_len = sum(test_labels[:,0]==1)
    
    c_e_len = sum(np.logical_and(test_labels[:,0]==0 , test_labels[:,1]==1))
    ...
    return abs(att_true - att_pred).item()
