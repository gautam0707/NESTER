...
if __name__ == '__main__':
    ...
    results_att_insample = []
    results_att_outsample = []
    
    args.train_valid_split = 0.8
    args.batch_size = 128
    args.learning_rate = 0.03
    args.neural_epochs = 10
    args.symbolic_epochs = 10
    args.max_depth = 3
    args.max_num_children = 10

    before = time.time()
    
    for i in range(10):
        print('dataset', i)
        data_train = pd.read_csv("data/jobs/Data/jobs_train_"+str(i)+".csv").reset_index(drop=True).sample(frac=1)
        data_test = pd.read_csv("data/jobs/Data/jobs_test_"+str(i)+".csv").reset_index(drop=True).sample(frac=1)
        ...
        train_data_ = data_train[data_cols].values
        test_data = data_test[data_cols].values

        train_labels = data_train["y_factual"].values
        train_labels_eval_ = data_train[["treatment","e", "y_factual"]].values
        test_labels = data_test[["treatment","e", "y_factual"]].values

        split = int(args.train_valid_split*len(train_data_))

        valid_data = train_data_[split:]
        train_data = train_data_[:split]

        valid_labels = train_labels[split:].reshape(-1,1)
        train_labels = train_labels[:split].reshape(-1,1)
        valid_labels_eval = train_labels_eval_[split:]
        train_labels_eval = train_labels_eval_[:split]
    ...
    #     assert train_data.shape[-1] == test_data.shape[-1] == args.input_size
        ...
        att = compute_pehe_score(best_program, train_data_, train_labels_eval_, args.output_type, args.output_size, args.num_labels, device)
        results_att_insample.append(att)

        # Evaluate best program on test set
        att = compute_pehe_score(best_program, test_data, test_labels, args.output_type, args.output_size, args.num_labels, device)
        results_att_outsample.append(att)
           
    print("ATT RESULTS")
    
    print("att in sample - mean: ", np.mean(results_att_insample), "std: " , np.std(results_att_insample))
    print("att out sample- mean: ", np.mean(results_att_outsample), "std: " , np.std(results_att_outsample))
