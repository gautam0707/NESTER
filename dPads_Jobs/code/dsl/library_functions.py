...
import pandas as pd
from .mmd_implementation import train_ipm
from .propensity_implementation import train_prop
from .sharedmlp import train_shared
...
class AffineFeatureSelectionFunction(AffineFunction):
    ...
    def execute_on_batch(self, batch, batch_lens=None):
#         assert len(batch.size()) == 2
        features = torch.index_select(batch.to(device), 1, self.feature_tensor)
        remaining_features = batch[:,self.full_feature_dim:].to(device)
...
class ConvergeNormalFunction(LibraryFunction):

    def __init__(self, input_size, output_size, num_units):
        super().__init__({}, "atom", "atom", input_size, output_size, num_units, name="ConvergeNormal", has_params=True)

    def init_params(self):
        self.parameters = { "mean" : torch.rand(self.input_size, requires_grad=True, device=device), "std": torch.rand(self.input_size, requires_grad=True, device=device)}
        self.linear_layer = nn.Linear(self.input_size, self.output_size, bias=False).to(device)

    def execute_on_batch(self, batch, batch_lens=None):
        #temp = phi(torch.tensor(batch.clone().detach(), dtype=torch.float))
        temp = phi(torch.tensor(batch.to(device), dtype=torch.float))
        return self.linear_layer(temp.type(torch.float))
class Mlp(AffineFeatureSelectionFunction):
    def __init__(self, input_size, output_size, num_units):
        self.full_feature_dim = 0 # this will indicate additional_inputs = 0 in FeatureSelectionFunction
        self.feature_tensor = torch.arange(input_size) # selects all features by default
        super().__init__(input_size, output_size, num_units, name="MLP")

class SharedMlp(LibraryFunction):
    def __init__(self, input_size, output_size, num_units):
        super().__init__({}, "atom", "atom", input_size, output_size, num_units, name="SharedMLP", has_params=True)
        self.idx=0
    def init_params(self):
        self.parameters = { "mean" : torch.rand(self.input_size, requires_grad=True, device=device), "std": torch.rand(self.input_size, requires_grad=True, device=device)}
        self.linear_layer = nn.Linear(self.input_size, self.output_size, bias=False).to(device)
        self.idx=0
    def execute_on_batch(self, batch, batch_lens=None):
        phi = train_shared(torch.tensor(batch.clone().detach().requires_grad_(True).to(device), dtype=torch.float))
        return self.linear_layer(phi.type(torch.float))
    
class Align(LibraryFunction):
    def __init__(self, input_size, output_size, num_units):
        super().__init__({}, "atom", "atom", input_size, output_size, num_units, name="Align", has_params=True)
        self.idx=0
    def init_params(self):
        self.parameters = { "mean" : torch.rand(self.input_size, requires_grad=True, device=device), "std": torch.rand(self.input_size, requires_grad=True, device=device)}
        self.linear_layer = nn.Linear(self.input_size, self.output_size, bias=False).to(device)
        self.idx=0
    def execute_on_batch(self, batch, batch_lens=None):
        phi = train_ipm(torch.tensor(batch.clone().detach().requires_grad_(True).to(device), dtype=torch.float))
        return self.linear_layer(phi.type(torch.float))
    
class Propensity(LibraryFunction):
    def __init__(self, input_size, output_size, num_units):
        super().__init__({}, "atom", "atom", input_size, output_size, num_units, name="Propensity", has_params=True)
        self.idx=0
    def init_params(self):
        self.parameters = { "mean" : torch.rand(self.input_size, requires_grad=True, device=device), "std": torch.rand(self.input_size, requires_grad=True, device=device)}
        self.linear_layer = nn.Linear(self.input_size, self.output_size, bias=False).to(device)
        self.idx=0
    def execute_on_batch(self, batch, batch_lens=None):
        phi = train_prop(torch.tensor(batch.clone().detach().requires_grad_(True).to(device), dtype=torch.float))
        return self.linear_layer(phi.type(torch.float))
class AffineTreatmentSelectionFunction(AffineFunction):
    def __init__(self, input_size, output_size, num_units, name="AffineTreatmentSelection"):
        
        self.full_feature_dim = input_size
        self.feature_tensor = torch.arange(1).to(device)
        additional_inputs = input_size - self.full_feature_dim
    ...
    def execute_on_batch(self, batch, batch_lens=None):
        assert len(batch.size()) == 2
        features = torch.index_select(batch.to(device), 1, self.feature_tensor)
        remaining_features = torch.zeros_like(batch[:,self.full_feature_dim:])
        return self.linear_layer(torch.cat([features.to(device).type(torch.float), remaining_features.to(device).type(torch.float)], dim=-1))